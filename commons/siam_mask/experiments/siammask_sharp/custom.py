...
from commons.siam_mask.experiments.siammask_sharp.resnet import resnet50
from commons.siam_mask.models.features import MultiStageFeature
from commons.siam_mask.models.mask import Mask
from commons.siam_mask.models.rpn import RPN, DepthCorr
from commons.siam_mask.models.siammask_sharp import SiamMask
...
class Custom(SiamMask):
    ...
    def track_refine(self, pos, batch_i: int = None):
        f = self.feature
        corr = self.corr_feature
        if batch_i is not None:
            f = tuple(_f[batch_i:batch_i + 1] for _f in f)
            corr = corr[batch_i:batch_i + 1]
        pred_mask = self.refine_model(f, corr, pos=pos, test=True)
        return pred_mask
